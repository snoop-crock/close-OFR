...
# Загрузка данных
df = pd.read_excel('Dataset_Small_OFR_crop.xlsx')

# Предположим, что ваш набор данных имеет целевую переменную 'q_i', а остальные - признаки
...
# Вычислим среднее значение целевой переменной для процентных метрик
y_mean = np.mean(y)
...
ws_metrics.append(['Trial', 'MSE', 'MSE%', 'RMSE', 'RMSE%',
                  'MAE', 'MAE%', 'R2', 'Spearman'])
...
def objective(trial):
    ...
    mse_percent = (mse / (y_mean**2)) * 100
    rmse_percent = (rmse / y_mean) * 100
    mae_percent = (mae / y_mean) * 100

    # Логирование в Excel
    ws_metrics.append([trial.number, mse, mse_percent, rmse,
                      rmse_percent, mae, mae_percent, r2, spearman])
...
# Вычисляем процентные метрики для финальной модели
mse_percent = (mse / (y_mean**2)) * 100
rmse_percent = (rmse / y_mean) * 100
mae_percent = (mae / y_mean) * 100

print("\nFinal Model Metrics:")
print(f"MSE: {mse:.4f} ({mse_percent:.2f}%)")
