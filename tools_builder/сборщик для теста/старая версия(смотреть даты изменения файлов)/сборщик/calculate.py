...
class Calc:
    def __init__(self, well, gwc):
        self.results_contour, self.results_well = self.calculate_min_distances(well, gwc)
    ...
    def get_H_to_GWC(self, well: pd.DataFrame, GWC: float) -> pd.DataFrame:
        ...
        min_well = well.groupby("Well")["z"].apply(lambda x: find_closest(x)).reset_index(drop=True)
    ...
    def calculate_min_distances(self, well: pd.DataFrame, gwc: pd.DataFrame):
        ...
        for well_id, points in wells[['Well', 'Points']].itertuples(index=False):
            ...
            distances_to_contour = [contour_tree.query(point)[0] for point in points]
        ...
        def sorter(df):
            ...
            df_sorted = df.sort_values(by='Numeric').drop(columns='Numeric').reset_index(drop=True)
            return df_sorted
    ...
    def get_kh_eff_shrt(self, kh_df: pd.DataFrame) -> pd.DataFrame:
        def get_eff_kh(secondary_wells):
            ...
            return eff_values
        eff_kh_df = pd.DataFrame(self.results_well['Shrt_well'].apply(get_eff_kh).tolist(), columns=['kHmin1', 'kHmin2', 'kHmin3'])
    ...
    def get_OFR(self, ofr: pd.DataFrame, basedir:str) -> pd.DataFrame:
        ...
        if num_wells > num_ofr:
            messagebox.showinfo("Информация", f'Не хватает значений в файле {basedir}ofr')
            exit()
...
def create_excel(basedir: str = ''):
    try:
        ...
        df_combined = pd.concat([
            ...
            calc.get_kh_eff(field.avg_f_perm_zone_well, field.kh_zone_well)['kh_eff'],
            calc.get_Hwell(field.all_gas_ass_h),
            calc.get_NTG(field.kh_zone_well, calc.get_Hwell(field.all_gas_ass_h)),
            ...
            calc.get_kh_eff_shrt(calc.get_kh_eff(field.avg_f_perm_zone_well, field.kh_zone_well)),
