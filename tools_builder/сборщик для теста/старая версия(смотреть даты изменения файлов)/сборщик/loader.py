...
class Field:
    ...
    def _avg_perm_zone_well(self):
        ...
        df = pd.read_csv(f'{self.dirname}avg_perm_zone_well', sep="\t", header=0, skiprows=skip_rows)
        ...
        result = pd.DataFrame(reshaped, columns=[f"kH_{i}" for i in range(reshaped.shape[1])])
    ...
    def _kh_zone_well(self):
        ...
        df = pd.read_csv(f'{self.dirname}kh_zone_well', sep="\t", header=0, skiprows=[1])
    ...
    def _all_gas_ass_h(self):
        ...
        df = pd.read_csv(f'{self.dirname}all_gas_ass_h', sep="\t", header=0, skiprows=[1])
        df.columns = df.columns.str.replace(r"\s*\(.*?\)", "", regex=True)
        df.rename(columns={"Дискретное свойство": "Well"}, inplace=True)
        df = df.map(self.__safe_convert)
        return df
    ...
    def _avg_f_perm_zone_well(self):
        ...
        df = pd.read_csv(f'{self.dirname}avg_f_perm_zone_well', sep="\t", header=0, skiprows=[1])
        df.columns = df.columns.str.replace(r"\s*\(.*?\)", "", regex=True)
        df.rename(columns={"Дискретное свойство": "Well"}, inplace=True)
        df = df.map(self.__safe_convert)
        return df
    ...
    def _GWC(self):
        ...
        df = pd.read_csv(f'{self.dirname}GWC', sep=" ", names=['x', 'y', 'z'], header=None)
        df = df.map(self.__safe_convert)
        return df
    ...
    def _well(self):
        ...
        for index, item in enumerate(temp_data[:]):
            ...
            temp_data[index] = "\t".join([matches, item.replace('       ', '\t')])
        ...
        df = pd.read_csv(f"{self.dirname}well_temp", sep='\t', names=['Well', 'x', 'y', 'z', 'md'], header=None, skiprows=skip_rows)
    ...
    def _field(self):
        ...
        df.columns = df.columns.str.replace(r"\s*\(.*?\)", "", regex=True)
        df.rename(columns={"Проницаемость по X": "INIT_PERMX", "Запасы газа": "gas_volume"}, inplace=True)
    ...
    def _ofr(self):
        ...
        normalized_data = VAR_columns.apply(lambda row: row / row.sum(), axis=1)
